...
export function ReadingProgress({ cycle = 2, totalCycles = 7, todayTarget = 100, todayCompleted = 42 }: ReadingProgressProps) {
  ...
  return (
    <section className="relative w-full rounded-[10px] border border-line bg-surface p-4" aria-labelledby="reading-progress-title">
      ...
      <div className="mt-3 flex items-baseline gap-3"><strong className="text-lg font-semibold text-text">{todayCompleted} / {todayTarget}語</strong><span className="text-xs text-text-muted">{cycle} / {totalCycles}セット</span></div>
      <div className="mt-3 block h-[5px] w-full overflow-hidden rounded-full bg-line" role="progressbar" aria-valuenow={todayCompleted} aria-valuemin={0} aria-valuemax={todayTarget} aria-label={`今日の読解 ${todayCompleted} / ${todayTarget}語`}><span className="block h-full rounded-full bg-accent transition-[width] duration-300" style={{ width: `${progress}%` }} /></div>
    </section>
  )
}
