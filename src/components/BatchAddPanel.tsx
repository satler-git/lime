    ...
    <section className="max-w-[620px] rounded-[10px] border border-line bg-surface p-5" aria-labelledby="batch-title">
      ...
      <div className="flex flex-col gap-[5px]">
        {candidates.map((candidate) => {
          ...
          return (
            <button className={`flex min-h-12 w-full cursor-pointer items-center gap-[9px] rounded-[7px] border border-transparent bg-surface-raised p-[6px_9px] text-left transition-[background-color,border-color] duration-120 hover:bg-surface-hover disabled:cursor-wait disabled:opacity-60 ${isSelected ? 'border-[rgba(194,230,111,.4)] bg-[rgba(194,230,111,.11)]' : ''}`} key={candidate.word} type="button" aria-pressed={isSelected} onClick={() => onToggle?.(candidate.word)} disabled={disabled || loading}>
