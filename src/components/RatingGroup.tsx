...
export function RatingGroup({ value, onChange, compact = false }: RatingGroupProps) {
  ...
  return (
    <div className="mt-[9px] grid grid-cols-4 gap-[5px]" role="group" aria-label="単語の理解度を評価">
      {options.map(({ value: optionValue, label, hint, icon: Icon }) => (
        <button className={`flex min-h-[60px] cursor-pointer flex-col items-center justify-center gap-[3px] rounded-[7px] border border-transparent bg-surface p-[7px_3px] ${colorClasses[optionValue]} transition-[background-color,border-color,transform] duration-120 hover:bg-surface-hover active:scale-[.96] ${value === optionValue ? 'border-current bg-current text-surface-raised' : ''}`} key={optionValue} type="button" aria-pressed={value === optionValue} onClick={() => onChange?.(optionValue)}>
